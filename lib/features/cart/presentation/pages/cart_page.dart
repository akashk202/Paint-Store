...
class _CartPageState extends ConsumerState<CartPage> {
  ...
  void _updateQuantity(String productKey, int newQuantity) {
    ...
    if (newQuantity > 0) {
      ...
      ref.read(cartNotifierProvider.notifier).removeItem(productKey);
      if (mounted) {
  ...
  void _updateSelectedSize(String productKey, PackSize newPackSize) {
    if (_auth.currentUser == null) return;
    ref.read(cartNotifierProvider.notifier).changeSize(
  ...
  Widget _buildCartItem(CartItemDetails item) {
    ...
    return Dismissible(
      ...
      onDismissed: (direction) {
        ref.read(cartNotifierProvider.notifier).removeItem(item.productKey);
      },
