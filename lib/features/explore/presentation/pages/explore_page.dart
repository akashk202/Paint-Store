...
import 'package:c_h_p/features/product/data/models/product_model.dart';import 'package:c_h_p/features/explore/data/models/explore_product_model.dart';
...
class _ExploreProductPageState extends ConsumerState<ExploreProductPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(exploreNotifierProvider.notifier).precacheHeroImages(context);
      // Trigger initial recommended load via Notifier (idempotent)
      ref.read(exploreNotifierProvider.notifier).loadRecommended(limit: 10);
    });
  }
  ...
  Widget _buildRecommendedSection() {
    final vmState = ref.watch(exploreNotifierProvider);
              ...
              return RecommendedProductCard(
                ...
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => ProductDetailPage(product: _toProduct(p)),
                    ),
                  );
                },
              );
