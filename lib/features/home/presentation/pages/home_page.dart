...
import 'package:c_h_p/features/home/presentation/providers/home_providers.dart';
import 'package:c_h_p/features/home/presentation/widgets/unread_badge.dart';
...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  List<HomeProductEntity> _suggestions = [];
  Timer? _debounce;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(homeNotifierProvider.notifier).loadAllProducts();
      final user = FirebaseAuth.instance.currentUser;
      if (user != null) {
        ref.read(homeNotifierProvider.notifier).observeUnread(user.uid);
  ...
  Future<void> _fetchAllProductsForSearch() async {
    // Legacy method retained for compatibility; now delegates to Notifier
    try {
      await ref.read(homeNotifierProvider.notifier).loadAllProducts();
      final vmProducts = ref.read(homeNotifierProvider).products;
      if (mounted) {
        setState(() {
    ...
    return AppBar(
      ...
      actions: [
        Builder(builder: (context) {
          ...
          if (currentUser != null) {
            ref.read(homeNotifierProvider.notifier).observeUnread(currentUser.uid);
          }
          final unreadCount = ref.watch(homeNotifierProvider).unreadCount;
          return Stack(
  ...
  void _onSearchChanged() {
    ...
    _debounce = Timer(const Duration(milliseconds: 300), () {
      ...
      ref.read(homeNotifierProvider.notifier).loadAllProducts();
      final products = ref.read(homeNotifierProvider).products;

      final rawQuery = _searchController.text.trim();
      final q = rawQuery.toLowerCase();

      List<HomeProductEntity> filtered = [];
  ...
  Product _toProduct(HomeProductEntity entity) {
    return HomeProductModel.fromEntity(entity).toProduct();
  }
